...
{
    WINRT_EXPORT template <std::size_t Size, typename T, std::size_t... Index>
    constexpr std::array<T, Size> to_array(T const* value, std::index_sequence<Index...> const) noexcept
    {
        return { value[Index]... };
    }

    WINRT_EXPORT template <typename T, std::size_t Size>
    constexpr auto to_array(std::array<T, Size> const& value) noexcept
    {
        return value;
    }

    WINRT_EXPORT template <std::size_t Size>
    ...
    WINRT_EXPORT template <typename T, std::size_t LeftSize, std::size_t RightSize, std::size_t... LeftIndex, std::size_t... RightIndex>
    ...
    WINRT_EXPORT template <typename T, std::size_t LeftSize, std::size_t RightSize>
    ...
    WINRT_EXPORT template <typename T, std::size_t LS, std::size_t RS, std::size_t... LI, std::size_t... RI>
    ...
    WINRT_EXPORT template <typename T, std::size_t LS, std::size_t RS>
    ...
    WINRT_EXPORT template <typename T, std::size_t S, std::size_t... I>
    constexpr std::array<T, S> to_zarray_base(T const(&value)[S], std::index_sequence<I...> const) noexcept
    {
        return { value[I]... };
    }

    WINRT_EXPORT template <typename T, std::size_t S>
    ...
    WINRT_EXPORT constexpr std::array<std::uint8_t, 4> to_array(std::uint32_t value) noexcept
    {
    ...
    WINRT_EXPORT constexpr std::array<std::uint8_t, 2> to_array(std::uint16_t value) noexcept
    {
        return { static_cast<std::uint8_t>(value & 0x00ff), static_cast<std::uint8_t>((value & 0xff00) >> 8) };
    }

    WINRT_EXPORT constexpr auto to_array(guid const& value) noexcept
    {
    ...
    WINRT_EXPORT constexpr std::uint32_t to_guid(std::uint8_t a, std::uint8_t b, std::uint8_t c, std::uint8_t d) noexcept
    {
    ...
    WINRT_EXPORT constexpr std::uint16_t to_guid(std::uint8_t a, std::uint8_t b) noexcept
    {
    ...
    WINRT_EXPORT constexpr std::uint32_t endian_swap(std::uint32_t value) noexcept
    {
        return (value & 0xFF000000) >> 24 | (value & 0x00FF0000) >> 8 | (value & 0x0000FF00) << 8 | (value & 0x000000FF) << 24;
    }

    WINRT_EXPORT constexpr std::uint16_t endian_swap(std::uint16_t value) noexcept
    {
        return (value & 0xFF00) >> 8 | (value & 0x00FF) << 8;
    }

    WINRT_EXPORT constexpr guid endian_swap(guid value) noexcept
    {
    ...
    WINRT_EXPORT constexpr guid set_named_guid_fields(guid value) noexcept
    {
    ...
    WINRT_EXPORT template <typename T, std::size_t Size, std::size_t... Index>
    ...
    WINRT_EXPORT constexpr auto sha1_rotl(std::uint8_t bits, std::uint32_t word) noexcept
    {
        return  (word << bits) | (word >> (32 - bits));
    }

    WINRT_EXPORT constexpr auto sha_ch(std::uint32_t x, std::uint32_t y, std::uint32_t z) noexcept
    {
        return (x & y) ^ ((~x) & z);
    }

    WINRT_EXPORT constexpr auto sha_parity(std::uint32_t x, std::uint32_t y, std::uint32_t z) noexcept
    {
        return x ^ y ^ z;
    }

    WINRT_EXPORT constexpr auto sha_maj(std::uint32_t x, std::uint32_t y, std::uint32_t z) noexcept
    {
        return (x & y) ^ (x & z) ^ (y & z);
    }

    WINRT_EXPORT constexpr std::array<std::uint32_t, 5> process_msg_block(std::uint8_t const* input, std::size_t start_pos, std::array<std::uint32_t, 5> const& intermediate_hash) noexcept
    {
    ...
    WINRT_EXPORT constexpr std::array<std::uint8_t, 8> size_to_bytes(std::size_t size) noexcept
    {
        return
        {
    ...
    WINRT_EXPORT template <std::size_t Size, std::size_t RemainingSize, std::size_t... Index>
    ...
    WINRT_EXPORT template <std::size_t InputSize, std::size_t RemainderSize>
    ...
    WINRT_EXPORT template <std::size_t... Index>
    ...
    WINRT_EXPORT constexpr auto get_result(std::array<std::uint32_t, 5> const& intermediate_hash) noexcept
    {
    ...
    WINRT_EXPORT template <typename TArg, typename... TRest>
...
#pragma warning(suppress: 4307)
    ...
    WINRT_EXPORT constexpr std::size_t to_utf8_size(wchar_t const value) noexcept
    {
        if (value <= 0x7F)
        {
            return 1;
    ...
    WINRT_EXPORT constexpr std::size_t to_utf8(wchar_t const value, char* buffer) noexcept
    {
        if (value <= 0x7F)
        {
