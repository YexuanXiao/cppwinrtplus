...
WINRT_EXPORT namespace winrt::impl {
    ...
    template <typename Handler, typename Dispatcher, typename Priority, typename Scheduler>
    [[nodiscard]] auto resume_foreground_common(Dispatcher const& dispatcher, Priority priority, Scheduler scheduler)
	{
		struct awaitable {
        private:
			...
			impl::dispatched_handler_state m_state;
        public:
			awaitable(Dispatcher const& d, Priority p, Scheduler s) noexcept :
                m_dispatcher(d), m_priority(p), m_scheduler(s)
            {
            }
			...
			void await_suspend(std::coroutine_handle<> handle) {
				...
				m_scheduler(m_dispatcher, m_priority, handler);
		...
		return awaitable{ dispatcher, priority, scheduler };
	}
}